"""Generate the Gaussian-mixture samples used by the hNDE analyses.

The signal and background are correlated, multimodal Gaussian mixtures with a
shared broad component that guarantees overlapping support. Nominal samples
and detector-scale variations are written in bounded batches, so peak memory
does not grow with the requested event count.
"""
...
# Write each nominal sample before generating the next one. For the
...
if args.with_systematics:
    ...
    # memory. Other analyses retain the nominal-only default.
